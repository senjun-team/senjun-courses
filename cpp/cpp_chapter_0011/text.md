...
В C++ возможно и **явное приведение типов** (explicit cast). Есть специальная языковая конструкция и встроенные функции для указания, к какому типу требуется привести значение. Пример: {#block-static-cast}
