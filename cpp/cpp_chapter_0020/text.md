...
Тип [std::string](https://en.cppreference.com/w/cpp/string/basic_string) реализует строку, не привязанную к кодировке. Каждый ее символ занимает ровно 1 байт.
...
Из примера видно, что для обращения к символу строки по индексу используется оператор `[]`. Индексация начинается с нуля.
...
- `insert()` вставляет заданное количество символов после указанного индекса: `s.insert(1, 2, '8') // e88xample`.
