...
Имя _должно начинаться_ с буквы латинского алфавита или символа подчеркивания `_`: `i`, `SearchEngine`, `connect_to_db`, `_abs_val`.
...
Поэтому при создании переменной [обязательно задавайте ей значение.](https://isocpp.github.io/CppCoreGuidelines/CppCoreGuidelines#es20-always-initialize-an-object)
...
Тип `char` представляет собой целое число, которое _можно_ трактовать как ASCII-код. Поэтому в обоих случаях переменная `letter` содержит одно и то же значение — число 83, в ASCII-таблице соответствующее заглавной букве S латинского алфавита.
...
Вы уже познакомились с [оператором присваивания](https://en.cppreference.com/w/cpp/language/operator_assignment) `=`. Рассмотрим еще несколько категорий операторов, которые пригодятся в первую очередь.
...
- `*` — умножение. `3 * 7 == 21`.
- `/` — деление.  `10 / 4 == 2`.
- `%` — получение модуля числа (остатка от деления). `11 % 3 == 2`.
...
В чем дело? Вначале выполнится постфиксный оператор и вернет неизменяемую копию переменной: `--(i++)`. А так как неизменяемое значение нельзя уменьшить на 1, компилятор прервет сборку программы с ошибкой:
...
- `a++`, `a--` — постфиксные инкремент и декремент.
...
Ремарка: это пример плохого кода. В реальных проектах таких трудночитаемых конструкций избегают. Однако они встречаются на собеседованиях. {.task_text}
