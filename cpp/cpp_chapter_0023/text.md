...
Хранение прав на файл в Unix-стиле — лишь одно из многих применений побитовых операторов для упаковки значений. Другие классические примеры — хранение трех компонент цвета в модели RGB или 4-х октетов в IPv4-адресе.
...
Напишите функцию, которая проверят, включен ли `n`-ный бит числа `x`. Не забудьте суффикс `uz` для литерала. Индексация битов идет справа налево и начинается с нуля. {.task_text}
