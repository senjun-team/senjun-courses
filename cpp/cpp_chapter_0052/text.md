...
В круглых скобках блока `catch` указывается объект исключения. Между именем объекта и его классом ставится символ амперсанда `&`. Это означает, что объект передаётся по ссылке, а не по значению. [В следующих главах](/courses/cpp/chapters/cpp_chapter_0141/) вы узнаете, что такое ссылки. А пока просто не забывайте ставить `&` перед именем исключения в блоке `catch`.
