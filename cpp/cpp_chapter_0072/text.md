...
Решите эту задачу с использованием побитового оператора XOR `^`. Если не получается догадаться до решения, воспользуйтесь подсказкой. {.task_text}
