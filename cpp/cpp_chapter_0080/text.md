...
В этой главе мы рассмотрим только алгоритмы, которые пригодятся в первую очередь.
...
Напишите функцию `count_vals()`. Она принимает отсортированный вектор и значение. Функция должна вернуть, сколько раз оно встречается в векторе. {.task_text}
