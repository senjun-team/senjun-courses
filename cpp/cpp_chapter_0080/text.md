...
Этот код делает то же самое, но выглядит короче и работает быстрее. Откройте оба примера в Playground и убедитесь в этом. Похожий пример с объяснениями причин ускорения мы [приводили,](/courses/cpp/chapters/cpp_chapter_0010/#block-zero-overhead) когда описывали одно из достоинств C++ — абстракции с нулевым оверхедом. 
...
Стандарт C++ требует выполнения этих условий для компараторов, передаваемых в алгоритмы. Вызов алгоритма с некорректным компаратором приводит к UB.

Компаратор, выполняющий эти условия, реализует строгий слабый порядок (strict weak ordering).
...
Для любопытных: самостоятельно разберитесь, в чем разница между `std::accumulate()` и `std::reduce()`.
...
- Алгоритмы стандартной библиотеки — это шаблонные функции для поиска, сортировки, копировния диапазонов и других полезных операций.
