...
Переменные со статическим временем жизни находятся в специальной области памяти, которая инициализируется _до_ входа в `main()`. Она располагается в секции с кодом программы и на картинке выше помечена зеленым.
...
Каждый блок кода создает новую область видимости. Объявленные внутри него переменные недоступны снаружи. Попытка обращения к ним извне приведет к ошибке компиляции.
...
- Для той же цели в условии управляющих конструкций, в том числе `while`, можно просто объявить и проинициализировать переменную, и она неявно приведется к `bool`.
