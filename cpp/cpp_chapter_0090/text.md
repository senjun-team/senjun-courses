...
Исполняемый файл на C++ — это бинарный файл, разбитый на секции. Можно назвать его образом программы. Когда программа запускается, ОС загружает ее образ в оперативную память. Под программу выделяется _адресное пространство._ Это абстракция физической памяти, которую ОС отдает процессу.
...
В отличие от локальных, глобальные переменные можно не инициализировать. Хотя делать так не рекомендуется. Глобальные переменные заполняются нулями, если у них нет конструктора:
...
Статическими можно делать не только локальные, но и глобальные переменные. Однако для глобальных переменных это имеет иное значение. Какое, вы узнаете в главе про сборку проекта.
...
public:
    ...
    void add_command(std::string cmd, std::string description)
    {
        cmd_descriptions[cmd] = description;
    }
...
- Автоматическое время жизни бывает у значений параметров функций и локальных переменных.
- Механизм RAII основан на времени жизни автоматических переменных.
