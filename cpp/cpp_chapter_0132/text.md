...
Инициализация по умолчанию (default-initialization) объектов классов приводит к вызову конструктора по умолчанию. Если этот конструктор не задан разработчиком явно, то компилятор его генерирует. Сгенерированный конструктор просто вызывает _конструкторы по умолчанию_ для всех полей класса.
...
- Поля инициализируются в порядке их объявления. В примере выше сначала инициализируется `requests_count`, затем — `user_id`.
- Это происходит _до_ вызова [конструктора.](/courses/cpp/chapters/cpp_chapter_0050/#block-constructors-destructors})
- Значение, заданное полю при прямой инициализации, можно переопределить в конструкторе.
...
Запомните правило: поля инициализируются в порядке своего объявления. А вовсе не в порядке перечисления в списке инициализации полей. Если они не совпадают, компилятор генерирует [предупреждение:](https://clang.llvm.org/docs/DiagnosticsReference.html#wreorder-ctor)
