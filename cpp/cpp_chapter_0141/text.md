...
Любое значение, выражение или функция имеет свой тип. Даже если он не задан явно, то все равно выводится компилятором:

```cpp
// Тип литерала 34.2 - double. Тип литерала 8 - int.
// Так как в делении присутствует double,
// то результат деления  - тоже double, а не int.
std::println("{}", 34.2 / 8);
```
```
4.275
...
- [Составные типы](https://timsong-cpp.github.io/cppwp/n4868/basic.compound) (compound types) основаны на других типах. Например, классы и функции.
...
Размер этих типов имеют фиксирован и зависит от реализации компилятора, ОС и целевой платформы. Выяснить размер можно с помощью оператора [sizeof](https://en.cppreference.com/w/cpp/language/sizeof.html), который принимает тип и возвращает отводимое под него количество байт:
...
- `wchar_t` (wide character) — имеет тот же размер и наличие знака, что и _один из целочисленных типов_. На практике под Windows `wchar_t` поддерживает UTF-16 и имеет ширину 16 бит. А на остальных системах он поддерживает UTF-32 и имеет ширину в 32 бита.
- `char8_t` — добавлен в C++20 для работы с UTF-8.
...
- `float` — [одинарная](https://en.wikipedia.org/wiki/Single-precision_floating-point_format) точность. Чаще всего занимает 4 байта и обладает точностью 6-9 знаков после запятой.
- `double` — [двойная](https://en.wikipedia.org/wiki/Double-precision_floating-point_format) точность. Как правило занимает 8 байт и имеет точность примерно в 15 знаков.
- `long double` — повышенная точность. Размер и точность сильно разнятся в зависимости от реализации.
...
- Для создания псевдонима используются ключевые слова `typedef` и `using`.
- Чтобы задать систему счисления для целочисленного литерала, используется префикс. Например, `0x` для шестнадцатеричного значения.
- Чтобы явно указать тип литерала, используется один из суффиксов `l`, `u`, `z` или их сочетание.
