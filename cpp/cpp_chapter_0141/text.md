...
Перед вами функция `naive_hash()` из [Путеводителя C++ программиста по неопределённому поведению.](https://github.com/Nekrolm/ubbook/blob/master/numeric/overflow.md) Она неправильно считает хеш от строки. В ней допущено UB, а конкретно — переполнение знакового целого. Переполнение происходит моментально, например при вызове `naive_hash("*")`. В переменную `h` типа `short` записывается слишком большое значение. Откройте задачу в плэйграунде и запустите проект. Так вы убедитесь в некорректном поведении функции. {.task_text}
...
Сравните погрешность, полученную при сохранении в `float` большого и маленького чисел. Она отличается на десять порядков! Откройте этот пример в плэйграунде и посмотрите, как поведёт себя тип для двойной точности `double`.
