...
Вы можете реализовать функцию двумя способами: через цикл или с использованием таких алгоритмов как [std::accumulate() или std::reduce().](/courses/cpp/chapters/cpp_chapter_0081/#block-accumulate-overload)
...
- Для чтения и записи полей могут быть реализованы методы произвольной сложности.
- Все поля экземпляра класса располагаются в единой области памяти. Однако они могут указывать на другие области динамической памяти, управление которой реализуется в методах класса.
...
Перед вами синтаксис объявления массива с именем `name`, состоящего из `n` элементов типа `type`:
...
```consoleoutput {.task_source #cpp_chapter_0142_task_0090}
```
. {.task_hint}
```cpp {.task_answer}
0
...
Напишите функцию `max_profit()`, на вход которой подается срез `prices`. Значение `prices[i]` описывает цену акций на `i`-ый день торгов. Нужно максимизировать прибыль, выбрав наиболее подходящие дни для покупки и продажи акций. Причем день для продажи должен идти после дня покупки. {.task_text}
