...
Символьный тип `char` является целочисленным, поэтому может выступать в качестве базового. В этом примере мы вызвали появившуюся в C++23 функцию [std::to_underlying()](https://en.cppreference.com/w/cpp/utility/to_underlying.html). Она приводит значение перечисления к базовому типу. До C++23 для этого пришлось бы воспользоваться `static_cast`:
...
Помимо сигнатуры у каждой функции есть тип. Он состоит из типов возвращаемого значения и параметров. Обычно он записывается в формате:
...
Чтобы компилятор мог выделить под массив память, его размер должен быть известен в момент компиляции, то есть быть константным.
...
Причем срезы можно создавать и над контейнерами. Это позволяет переиспользовать одни и те же функции для обработки сишных массивов и контейнеров:
