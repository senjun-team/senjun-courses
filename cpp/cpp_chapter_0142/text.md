...
Итак, мы вплотную подобрались к двум фундаментальным для C++ концепциям: ссылкам и указателям. Начнем с затравки, а детально рассмотрим их в следующей главе.
...
Амперсанд после типа означает, что перед вами не обычный тип, а ссылочный. В данном примере у переменной `n` тип `std::size_t &`: это ссылка на переменную `std::size_t`.

![Ссылка](https://raw.githubusercontent.com/senjun-team/senjun-courses/refs/heads/cpp-chapter-14/illustrations/cpp/reference.jpg) {.illustration}

Наиболее частый сценарий для использования ссылок — предотвращение лишнего копирования объектов. По умолчанию функции принимают параметры по значению (by value): при вызове функции в нее вместо исходных переменных попадают _копии._ Убедимся в этом:
...
Когда функция получает аргумент по ссылке, компилятор связывает существующую переменную с новым именем — параметром функции. То есть заводит для нее псевдоним. И функция получает доступ к исходному объекту вместо копии:
