...
Перечисление `Facility` содержит 261 константу по количеству компонентов системы, для которых нужно собирать метрики. Какой базовый тип подойдёт для `Facility` лучше всего? Выберите один из целочисленных типов [фиксированной ширины.](/courses/cpp/chapters/cpp_chapter_0141/#block-fixed-width) {.task_text}
...
В этом примере мы заполнили массив значениями сразу после создания. Если этого не сделать, то элементы будут [инициализированы по умолчанию.](/courses/cpp/chapters/cpp_chapter_0131/#block-default-initialization) Для фундаментальных типов это означает _отсутствие_ инициализации, которое приведёт к UB на строке с выводом массива в консоль.
