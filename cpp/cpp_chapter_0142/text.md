...
Память под сишные статические массивы выделяется [на стеке.](/courses/cpp/chapters/cpp_chapter_0090/#block-stack)
...
- Сишные статические массивы — это непрерывная область памяти на стеке, которую компилятор интерпретирует как подряд идущие элементы заданного типа.
