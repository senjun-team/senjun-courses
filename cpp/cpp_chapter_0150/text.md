...
Например, строка `"ReFeRenCe"` в нижнем регистре будет выглядить как `"reference"`. {.task_text}
...
- Ту же переменную, что была передена в функцию по ссылке в качестве аргумента.
...
Это выглядит необычно, но вызов метода, возвращющего ссылку, может стоять по левую сторону от оператора `=`:
...
Вот несколько сценариев, при которых ссылки незаменимы:
- Функция изменяет передаваемый в нее объект.
- В функцию попадает тяжелый объект, но она его не изменяет. В этом случае подойдет константная ссылка.
- Итерирование по тяжелым элементам контейнера.
