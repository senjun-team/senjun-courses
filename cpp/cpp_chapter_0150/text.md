...
Ссылку нельзя переназначить. Она инициализируется в момент своего объявления, и после этого применение к ней оператора `=` будет означать присваивание значения исходной переменной. Иными словами, все своё время жизни ссылка «смотрит» на один и тот же объект.
...
Функция должна вернуть слово, встречающееся в `text` чаще всего и не входящее в `stop_words`. Строка `text` состоит из слов, разделённых пробелами. Если она пустая, функция должна вернуть пустую строку. {.task_text}
...
- Передача _по константной сслыке_ для предотвращения копирования тяжёлого объекта.
