...
Главное при этом — помнить, что ссылка должна указывать на живой, еще не уничтоженный объект. Иначе вы получите висячую ссылку (dangling reference), которая смотрит на уже не существующий объект. Обращение по такой ссылке приведет к UB. Например:
...
int main()
{
    double & v = pyramid_volume(14.3, 6.2);
    std::println("{}", v);                // UB
}
```
