...
В современном C++ плохой практикой [считается](https://isocpp.github.io/CppCoreGuidelines/CppCoreGuidelines#rf-out) применение ссылок для возврата из функции нескольких значений. Как правило этот приём можно встретить в старом коде.
...
Временный объект (temporary object) — это неименованный объект, который создаётся компилятором для хранения некоего значения. Временные объекты живут недолго: как правило, до конца инструкции.
...
Почему для продления жизни временных объектов требуется именно _константная_ ссылка? Во-первых, семантически временный объект подразумевает именно read-only доступ. Попытка обратиться к нему на запись означает, что в логике программы не всё гладко. Во-вторых, для константного объекта компилятор гарантированно способен определить время жизни. А для не константного объекта такой гарантии нет.

### Висячие ссылки

Ссылка становится висячей (dangling reference), когда уничтожается или перемещается объект, на который она указывает. Обращение по такой ссылке — это UB. И одна из приводящих к этому распространенных ошибок — заведение ссылок на элементы контейнеров, которые могут быть перемещены в памяти.
...
- Время жизни ссылки не должно превышать время жизни объекта, на который она указывает. Иначе вы получите висячую ссылку, обращение по которой приведёт к UB.
