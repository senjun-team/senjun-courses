...
**Ссылка** (reference) — это псевдоним для существующей переменной.
...
Расстановка пробелов вокруг оператора ссылки `&` роли не играет. Допустимы и такие варианты объявления ссылки:
...
И, наконец, ссылку нельзя переназначить. Ссылка инициализируется в момент своего объявления. После этого применение к ней оператора `=` будет означать присваивание значения исходной переменной. Иными словами, все свое время жизни ссылка «смотрит» на один и тот же объект.
...
int main()
{
    double & v = pyramid_volume(14.3, 6.2); // v - висячая ссылка
    std::println("{}", v);                  // UB
}
```
...
- Переменную со [статическим временем жизни.](/courses/cpp/chapters/cpp_chapter_0090/#block-static-lifetime) Такие переменные создаются до входа в `main()` и разрушаются при завершении программы.
...
Например, у нас есть класс для хранения ключей и значений. Чтобы при обращении по ключу не создавать копию значения, метод `get()` возвращает ссылку на значение:
...
Основных сценариев использования ссылок всего два:
- Передача _по ссылке_ для изменения объекта.
- Передача тяжелого объекта _по константной сслыке_ для предотвращения лишнего копирования.

Как правило, тяжелыми считаются объекты, размер которых превышает одно-два машинных слова. [Машинное слово](https://ru.wikipedia.org/wiki/%D0%9C%D0%B0%D1%88%D0%B8%D0%BD%D0%BD%D0%BE%D0%B5_%D1%81%D0%BB%D0%BE%D0%B2%D0%BE) — это единица данных, которую процессор обрабатывает как единое целое. Длина машинного слова (то есть количество бит, которое оно занимает) определяется архитектурой процессора. Например, в архитектуре x86-64 машинное слово занимает 64 бита.

Это означает, что передавать в функцию по ссылке такие типы как `std::uint64_t`, `double` и `bool` бессмысленно. Зато `std::string`, `std::vector` и другие классы могут иметь большой размер, и их нужно передавать по ссылке.

Как эффективнее передавать этот тип в функцию — по значению или по константной ссылке? Введите `val` или `const ref`. {.task_text}
...
В современном C++ плохой практикой считается применение ссылок для имитации «возврата» из функции нескольких значений. Как правило этот прием можно встретить в старом коде в стиле Си.
...
Работать с функциями, которые по смыслу должны вернуть несколько значений, но по факту возвращают одно, а остальные _изменяют_ через параметры — крайне неудобно. Такой код выглядит странно. Пользуйтесь альтернативами:
...
- После создания ссылку нельзя переназначить.
- Константная ссылка (или ссылка на константный объект) — это ссылка, по которой можно только читать значение, но не изменять его.
- Если функция принимает параметр _по значению,_ то при ее вызове происходит копирование соответствующего аргумента. Если функция принимает параметр _по ссылке,_ то она работает с исходным объектом. Копирования не происходит.
- Время жизни ссылки не должно превышать время жизни объекта, на который она «смотрит». Иначе вы получите висячую ссылку, обращени по которой приведет к UB.
