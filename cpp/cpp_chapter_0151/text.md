...
В стандартной библиотеке есть RAII-обертки над сырыми указателями. Их называют умными указателями (smart pointers). В момент инициализации умный указатель выделяет память, а при вызове деструктора — освобождает ее. В новом коде предпочтение отдается умным указателям.

Тогда для чего же лично вам разбираться с сырыми указателями? 
...
Чем лучше вы разберетесь, как устроено управление динамической памятью, тем профессиональнее сможете писать на C++:
...
- Вы будете понимать, _почему_ у какого-то метода контейнера сложность — _амортизированная_ константа `O(1)`, а у другого — просто константа `O(1)`. А значит, вам проще будет составлять эффективные алгоритмы.

Указатели — ключ к более глубокому пониманию языка, к оптимизации потребления ресурсов и скорости выполнения. Поэтому, хоть и не поощряется использование указателей, знать про них — **обязательно.** Уже не говоря о том, что указатели есть в кодовой базе практически любого долгоживущего проекта.
