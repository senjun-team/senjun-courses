...
В главе про область видимости мы [разбирали,](/courses/cpp/chapters/cpp_chapter_0090/#block-stack-frame) что такое стек вызовов, база стека и вершина стека. А также какую информацию содержат его фреймы. Изобразим, как выглядит стек вызовов в три момента времени: когда выполняется тело `pyramid_volume()`, когда в `v` сохраняется результат этой функции, и когда `std::println()` обращается к `v`. 
