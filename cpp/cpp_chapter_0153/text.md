# Глава 15.3. Адресная арифметика

Низкоуровневое программирование на C++ сложно представить без адресной арифметики. Она нужна, чтобы эффективно перемещаться по непрерывной области памяти. Главное при этом — знать, элементы какого типа в ней расположены.
...
```consoleoutput {.task_source #cpp_chapter_0153_task_0010}
```
...
```consoleoutput {.task_source #cpp_chapter_0153_task_0020}
```
...
```cpp {.task_source #cpp_chapter_0153_task_0040}
...
```cpp {.task_source #cpp_chapter_0153_task_0050}
...
```consoleoutput {.task_source #cpp_chapter_0153_task_0060}
```
...
```cpp {.task_source #cpp_chapter_0153_task_0070}
...
Адресная арифметика позволяет через указатели перемещаться по сишным массивам. Получается, указатель — это частный случай итератора. Ведь к итераторам тоже применимы операции разыменования `*`, обращения к члену класса `->`, инкремент и декремент.

Указатель считается итератором [произвольного доступа](/courses/cpp/chapters/cpp_chapter_0061/#block-iterator-categories) (random access). А значит, указатели можно передавать во все алгоритмы стандартной библиотеки, принимающие итераторы!
...
- Указатели считаются итераторами произвольного доступа. Поэтому их можно передавать в алгоритмы стандартной библиотеки, принимающие итераторы.
