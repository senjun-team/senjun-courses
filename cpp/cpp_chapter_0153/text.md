...
Чем лучше вы представляете, как устроена память, тем эффективнее можете оптимизировать свой код. Для начала нужно [помнить,](/courses/cpp/chapters/cpp_chapter_0090/#block-memory) в каких областях памяти живут переменные.
...
У Статической и автоматической областей памяти крайне ограниченный размер. Под статическую память процесса как правило выделяется 2 Кб, а под автоматическую — 2 Мб. У вас не получится завести там действительно большие объекты. Для них предназначена _динамическая память._

Как же работает заполнение стандартных контейнеров десятками миллионов элементов? Когда вы внутри функции заводите вектор, переменная типа `std::vector` размещается в автоматической памяти (на стеке). Но у вектора есть приватное поле — указатель на кусок динамической памяти. Там и хранятся все элементы. Получается, что объект вектора со всеми полями находится на стеке. Но одно из полей ссылается на динамическую память. И управление этой памятью реализовано в методах вектора.

Чтобы управлять выделением и освобождением динамической памяти, в языке предусмотрены парные операторы:
- `new` и `delete` для штучных объектов.
- `new[]` и `delete[]` для массивов.

## Операторы new и delete

Временем жизни переменных в динамической памяти управляет разработчик. Оно _не завершается_ при выходе из области видимости. Переменная создается с помощью оператора [new](https://en.cppreference.com/w/cpp/language/new.html) и уничтожает оператором [delete](https://en.cppreference.com/w/cpp/language/delete.html). Работа с переменной организуется через указатель.
...
- Если вы вызовите `delete` _дважды,_ то получите **двойное освобождение памяти.** Это повреждение памяти, приводящее к любым последствиям. Иными словами, это типичное UB.
