...
Временем жизни переменных в динамической памяти управляет разработчик. Оно _не завершается_ при выходе из области видимости. Поэтому выделение и освобождение памяти — всегда парные действия. Если вы выделили под объект память, то должны ее освободить, когда объект перестанет использоваться. Выделение динамической памяти называется **аллокацией.**
...
На одну аллокацию памяти по указателю `p` должно приходиться ровно одно освобождение памяти.
- Если вы забудете освободить память, то получите **утечку памяти.** Занятая область памяти будет возвращена ОС только при завершении программы.
- Если освободите одну и ту же область памяти дважды, то получите **двойное освобождение памяти.** Это повреждение памяти, приводящее к любым последствиям. Иными словами, типичное UB.
...
В нем, помимо прочего, объявлены функции [malloc()](https://en.cppreference.com/w/cpp/memory/c/malloc.html) (memory allocation) и [free()](https://en.cppreference.com/w/cpp/memory/c/free.html) Это не единственные, но основные функции для управления памятью.
...
Одна из фичей, появившаяся еще на заре C++ — [пространства имен.](/courses/cpp/chapters/cpp_chapter_0053/) Они позволяют предотвращать конфликты имен и удобно группировать код. Разумеется, функции библиотеки рантайма Си были включены в пространсвто имен `std`. Это подталкивало знакомившихся с C++ сишников применять новые языковые возможности.
...
- Единственное, что делает `malloc()` — аллоцирует сырую память, в которой может находиться что угодно. Вы обязаны инициализировать ее вручную: `malloc()` не вызывает конструктор.
...
Обратите внимание метод `int & operator[](std::size_t i)`: он нужен для обращения к объекту `Vector` через квадратных скобок. Подробнее про операторы будет в следующих главах.

Также мы завели приватный метод `move()`, который переиспользуем для реаллокации массива.

Поведение методов класса `Vector` постараемся сделать максимально приближенным к [аналогичным методам](https://en.cppreference.com/w/cpp/container/vector.html) `std::vector`.
...
Не смотря на название, метод не очищает массив, а всего лишь помечает его пустым.
