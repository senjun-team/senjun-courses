...
Почему так происходит? При вызове `malloc()` ОС выделяет виртуальный адрес, но пока не привязывает к нему физическую память. Когда по этому адресу начинается запись данных, ОС пробует выделить реальную страницу физической памяти. Если на этот момент свободной памяти нет, ОС аварийно завершает программу. Причем не факт, что именно вашу.

Обещание памяти без гарантии ее налачия называется [memory overcommitment](https://en.wikipedia.org/wiki/Memory_overcommitment). Эта стратегия особенно свойственна Linux и позволяет гибко распределять ресурсы: ОС раздает процессам «кредиты» в надежде, что они не используют всю обещанную память одновременно. Если же процессы решают обналичить «чеки», а памяти не хватает, ОС принудительно завершает один из процессов.
...
- В C++ есть несколько способов для работы с динамической памятью:
    ...
    - Умные указатели.
- При нехватке памяти возможен overcommit: вызов `malloc()` или `new` завершается успешно, но при обращении к выделенной памяти ОС посылает процессу сигнал для завершения. Вывод: успешное завершение `malloc()` или `new` не гарантирует, что память действительно отдана процессу. Чтобы в этом убедиться, нужно обратиться ко всей выделенной памяти.
