...
Работать с указателями _неудобно и опасно,_ и в предыдущих главах вы точно прочувствовали это. Управлять через указатели памятью — тоже _неудобно и опасно._ Чрезвычайно легко совершить ошибку:
...
- `std::unique_ptr` для _одиночного_ владения объектом. В каждый момент времени на объект может указывать только один `std::unique_ptr`.
...
5. Объекта нет, но память под него выделена. Она не передана под управление `std::unique_ptr`, и возникает утечка.
...
- Если вы работаете с сишной библиотекой, не позволяйте сырым указателям расползтись по коду вашего проекта: изолируйте их и оборачивайте в умные указатели. Если потребуется, пишите RAII-классы - обертки.
