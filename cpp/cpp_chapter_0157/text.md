...
А вот пример объявления умного указателя на `int`:

```cpp
std::unique_ptr<int> p_val(new int{16});
```

Также можно создать умный указатель на сишный массив:
...
Класс [std::weak_ptr](https://en.cppreference.com/w/cpp/memory/weak_ptr.html) используется в связке с `std::shared_ptr` и не увеличивает его счетчик ссылок. Он нужен для предотвращения циклических зависимостей, когда два `std::shared_ptr` «смотрят» друг на друга, их счетчики не обнуляются, и поэтому указатели не могут быть удалены. Такие «мертвые петли» неизбежны между узлами двусвязных списков, деревьев и графов:
