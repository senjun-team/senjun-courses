...
Здесь мы создали в динамической памяти переменную типа `int`, присвоили ей значение 16 и создали на нее умный указатель.
...
Вы пишете библиотеку для сохранения скриншотов. Системная функция (допустим, из [WinAPI](https://ru.wikipedia.org/wiki/Windows_API) или [X11](https://www.x.org/releases/current/doc/)) возвращает вам сырой массив пикселей. Вы обернули его в `std::unique_ptr`, чтобы гарантировать очистку памяти, выделенной через `new[]`. Но в объявлении умного указателя допущена ошибка. Исправьте ее. {.task_text}
...
{
    // Представьте, что это вызов системной функции
...
Умный указатель `std::unique_ptr` нельзя копировать, и это логично. Если бы копирование было разрешено, то два умных указателя считали бы себя единственными владельцами одной и той же области памяти. Для ее освобождения каждый бы вызвал `delete`, что означало бы двойное освобождение памяти.

Объект копируется в двух случаях: при вызове конструктора копирования или оператора присваивания `=`. Оба помечены в классе `std::unique_ptr` удаленными, чтобы их нельзя было использовать:
...
Если `std::unique_ptr` организует одиночное владение объектом, то [std::shared_ptr](https://en.cppreference.com/w/cpp/memory/shared_ptr.html) — совместное. Несколько `std::shared_ptr` могут указывать на один и тот же объект. Для этого внутри класса ведется подсчет ссылок. Узнать значение счетчика можно вызовом метода `use_count()`:
...
- Если вы работаете с сишной библиотекой, не давайте сырым указателям расползтись по коду вашего проекта: оборачивайте их в умные указатели. Если потребуется, пишите RAII-классы - обертки.
