...
А это — абсолютно эквивалентный вариант записи:

```cpp
double get_median(double * arr, std::size_t len);
```

Поэтому если в функцию передается двумерный массив, это можно записать так:

```cpp
double max_element(double * arr[], std::size_t n, std::size_t m);
```

То есть мы передаем указатель на нулевой элемент массива, у которого каждый элемент — это тоже массив.
...
А вот — еще одна популярная задача про указатели с собеседований. Нужно написать функцию `get_intersection_val()`, которая принимает указатели на первые элементы двух списков. Функция должна вернуть указатель на элемент, на котором эти списки пересекаются. Если пересечения нет, функция возвращает `nullptr`. {.task_text}
...
В этом примере пересечения списков функция должна вернуть указатель на элемент со значением `8`: {.task_text}
...
ListNode * get_intersection_val(const ListNode * head_a, const ListNode * head_b)
{
...
Однако есть несколько сценариев, при которых вместо ссылки потребуется указатель:
