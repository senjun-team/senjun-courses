...
Статическая и автоматическая области памяти довольно ограничены. Например, под статическую память процесса может быть выделено 2 Кб, а под автоматическую — 2 Мб. У вас не получится завести там _действительно_ большие объекты. Для них предназначена динамическая память.

Как же тогда работает заполнение стандартных контейнеров десятками миллионов элементов? Когда вы внутри функции заводите вектор, переменная типа `std::vector` размещается в автоматической памяти (на стеке). Но в любой реализации у вектора есть приватное поле — указатель на область динамической памяти. Там и живут элементы вектора. Получается, что объект вектора со всеми полями находится на стеке. Но одно из полей — это указатель, ссылающийся на динамическую память. Чтобы управлять ее выделением и освобождением, в языке предусмотрены парные операторы:
- `new` и `delete` для объектов.
- `new[]` и `delete[]` для массивов.

## Операторы new и delete

Время жизни переменных в динамической памяти _не ограничено_ их областью видимости. Им управляет разработчик. Переменная создается с помощью оператора [new](https://en.cppreference.com/w/cpp/language/new.html) и уничтожает оператором [delete](https://en.cppreference.com/w/cpp/language/delete.html). Работа с переменной организуется через указатель.
...
1. На куче выделяется область памяти под тип `int`. Например, 4 байта. 
...
В момент вызова `delete x` выделенная память помечается свободной, а указатель `x` становится висячим (dangling pointer): он перестает указывать на корректную область памяти. Ведь в любой момент по этому адресу может быть записана другая переменная.

Если у типа есть конструктор, то он отрабатывает при вызове `new` сразу после выделения памяти. При вызове `delete` сначала вызывается деструктор, а потом освобождается память.
...
- Если вы не вызовите `delete` для созданного через `new` объекта, то получите **утечку памяти.** Выделенная память будет отдана ОС только при завершении программы.
- Если вы вызовите `delete` дважды, то получите **двойное освобождение памяти.** Это повреждение памяти, которое может привести к произвольным последствиям. Иными словами, это UB.
...
Теперь у нас есть все необходимое, чтобы реализовать примитивную, но вполне работоспособную реализацию вектора. Сначала опишем конструкторы, деструктор и приватные поля. А затем добавим несколько методов.
...
private:
    ...
    // Указатель на сишный массив
    int * m_elements = nullptr;
};
```

Мы завели перегрузку конструктора, позволяющую заводить вектор из `n` элементов со значением `val`. Пример его вызова:
