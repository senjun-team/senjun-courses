...
Но вначале создаётся массив с памятью `mem` и указатель на ячейку памяти `i`, равный нулю. Также нужен счётчик `i_command`, указывающий на текущую обрабатываемую команду. Так как в коде могут присутствовать скобки, то счётчик будет не только расти, но и уменьшаться:
...
Для обработки текущей команды подойдёт `switch-case`:
