...
Необходимо реализовать функцию `calc()`. Она принимает единственный аргумент — строку. Строка содержит алгебраическое выражение, состоящее из чисел, символов математических операций и скобок. Например:
...
Функция `calc()` должна вернуть вещественное число — результат вычисленного выражения. Либо бросить исключение типа [std::invalid_argument](https://en.cppreference.com/w/cpp/error/invalid_argument.html), если выражение составлено некорректно. Считаем, что входных данных, приводящих к делению на ноль, не будет.
...
Так как в этих четырех выражениях есть ошибки, функция `calc()` для них должна сгенерировать исключение `std::invalid_argument`.

## Тестирование

Вы можете дописать свои варианты алгебраических выражений в `main.cpp` и проверить, как ведет себя функция `calc()` на различных входных данных. Для этого нажмите кнопку «Запустить».

Когда функция `calc()` будет готова к тестированию, нажмите кнопку «Отправить на проверку». По ней запустятся юнит-тесты из файла `tests.cpp`. В этом же файле можно посмотреть ожидаемые значения функции для набора алгебраических выражений.

## Что может пригодиться

Если вы захотите, чтобы ваша реализация калькулятора была расширяемой и гибкой, то скорее всего вам не понравится такой код:
...
Он выглядит громоздко. Сравните:
...
Здесь `kOperators` — константный словарь, у которого ключи — строки, а значения — структуры, содержащие информацию про оператор: его приоритет и собственно реализацию `func`.
...
Чтобы присвоить полю `func` функцию, реализующую оператор, вовсе не обязательно писать ее самостоятельно:
...
Тогда инициализация словаря `kOperators` будет выглядеть примерно так:
...
В таблицу переходов можно было бы добавить столбец «Действие» и в нем описать функции, которые должны вызываться при переходе между состояниями. Например, при переходе в состояние «Ошибка» требуется прерывать обработку токенов и бросить исключение.
