...
Заводим переменную `res` типа `std::size_t` и инициализируем нулём. В ней мы будем накапливать результат целочисленного деления `a` на `b`.

В цикле, пока `a` больше или равно `b`, делаем три вещи:

[1] Находим `n` — количество бит, на которые можно сдвинуть `b` влево, чтобы вычесть из `a`. Для этого инициализируем `n` нулём. И заводим ещё одну переменную: `b_next`, которая изначально равна `b << 1`. И во вложенном цикле, пока выполняются условия `a >= b_next` и `b < b_next`, увеличиваем `n` на единицу, а `b_next` ещё раз сдвигаем вправо.
