...
int main()
{
    ...
    // Тест расчёта скользящего среднего для нескольких коротких последовательностей чисел
    "Simple cases"_test = [] {
    ...
    "Check solution running time against simple solution"_test = [] {
        ...
        // Допустимое замедление, включая погрешность вычисления на VPS
        expect(avg_user <= avg_senjun * 3.0) << "Your implementation of MovingAverage doesn't fit our timeouts. Please optimize it." << fatal;
    };
}
