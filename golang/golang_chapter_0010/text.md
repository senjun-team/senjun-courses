...
Существуют интерпретаторы языка Go, однако зачастую в них нет надобности, потому что программы на языке Go компилируется быстро. 
...
В процессе написания программ часто возникают объекты, на которых не ссылается ни один другой объект. Например, на локальную переменную по выходу из функции, в которой она объявлена, ссылок больше нет. Такие объекты называются недостижимыми. Сборщик мусора обеспечивает безопасную работу с памятью, очищая ее от недостижимых объектов автоматически. Это облегчает работу, однако не освобождает от необходимости помнить о возможных утечках памяти. Неиспользуемые объекты не будут уничтожены, если на них ссылаются другие объекты.
