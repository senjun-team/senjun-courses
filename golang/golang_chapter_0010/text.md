...
Во время работы программ часто возникают объекты, на которые не ссылается ни один другой объект. Например, в C++ такие объекты получаются в результате выделения памяти командами `new` или `malloc`. Если программист вовремя не очистит память с помощью `delete` или `free`, то остается объект, на который больше нет никаких ссылок. Это недостижимый объект. Сборщик мусора в Go обеспечивает безопасную работу с памятью, очищая ее от недостижимых объектов автоматически. Это облегчает работу, однако не освобождает от необходимости помнить о возможных утечках памяти. Неиспользуемые объекты не будут уничтожены, если на них ссылаются другие объекты.
