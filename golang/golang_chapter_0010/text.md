...
В Go также используются указатели в стиле C. Указатель — это переменная, в которой хранится адрес некоторой памяти. Адрес — это число, которое обозначает место в памяти, куда записывается значение. Например, переменная типа `int` может иметь адрес `0xc000010120`. Префикс `0x` означает, что число `c000010120` представлено в шестнадцатеричной системе счисления.

Указатель в Go — это всегда указатель на некоторый тип. Иными словами, указатель не может хранить адрес чего угодно. Указатель всегда хранит адрес переменной некоторого типа. Чтобы объявить переменную типа указатель на `string`, нужно поставить перед словом `string` символ `*`. Чтобы взять адрес переменной, необходимо перед именем переменной поставить символ `&`, а чтобы разыменовать указатель и получить значение необходимо поставить `*`. Арифметика указателей отсутствует во избежание ошибок.
...
Допишите функцию `swap()`, которая принимает на вход два указателя типа `string`, и меняет местами значения соответствующих переменных. Программа должна вывести сообщение `Hello, gophers!`. 

Легко поменять местами две переменные можно следующим образом:
```go
a := 5
b := 10
a, b = b, a
...
Получите значения соответствующих переменных через символ `*` и поменяйте их местами. {.task_hint}
