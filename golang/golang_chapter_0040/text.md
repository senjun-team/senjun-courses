...
Success!
```
В общем случае, в результате вызова `p := range N` переменная `p` пробегает все значения из интервала `[0; N)`. Правая граница не включается. В нашем случае переменная `i` пробегает значения `0,1,2`.
