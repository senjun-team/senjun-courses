...
Чтобы не указывать количество элементов массива `[5]int`, используется следующую запись:
...
В Go, если какая-либо из возвращаемых переменных не нужна, вместо имени можно поставить пустой идентификатор `_`. Например:
...
Массивы могут быть многомерными. Вот как можно объявить двумерный массив (матрицу) из чисел типа `int` и задать элементу этой матрицы с индексами `4`, `5` значение `250`:
...
Конечно, можно передать массив в функцию по указателю. Однако тип такого указателя зависит от размера массива, поскольку размер массива — часть его типа. Поэтому код получается хрупким.
...
2. Для обхода массива удобно использовать циклс с `range`. 
