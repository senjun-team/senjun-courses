...
В Go строка `string` представляет собой срез байтов, доступный только для чтения.
...
По этой причине проверять, является ли срез пустым, лучше через сравнение `len(slice) == 0`, а не `slice == nil`.
...
Важно! Как упоминалось ранее, срез — это всего лишь указатель, длина и емкость. Срез не создает копию базового массива или его части. Рассмотрим следующий пример:
