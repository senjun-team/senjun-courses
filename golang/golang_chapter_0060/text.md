...
В памяти `map` организована как массив из `bucket`. `Bucket` — это массив из восьми элементов «ключ — значение». 
Так происходит, потому что значения элементов `map` не адресуемы. Хеш-таблица перераспределяется, если число ее элементов превышает некоторый порог. Если бы можно было получить адрес значения элемента `map`, то после перераспределения он бы устарел и оказался недопустимым. 
