...
С использованием `piDigit()` реализуйте тело функции `countDigits()`. Она принимает количество цифр числа Пи после десятичной точки (`0 <= n <= 10`) и возвращает отображение с количеством раз, которое встретилась каждая цифра. Цифры, не встретившиеся ни разу, в отображение включать не нужно. {.task_text}
...
Отметим, что начиная с версии Go 1.12, отображения сортируются при выводе по ключам. Таким образом, можно было бы сравнить два отображения как строки:
