...
Заполнить отображение данными при его создании:
```go
...
Обратиться к элементу отображения по его ключу:
```go
...
Удалить элемент из отображения с помощью встроенной функции `delete`:

```go
delete(catsWeights, "Busa")
```

Узнать длину отображения с помощью встроенной функции `len()`:
...
Перебрать все пары «ключ-значение» с помощью цикла и ключевого слова `range`:
```go
...
7. Существуют специальные приемы, которые позволяют реализовать множества стандартными средствами языка Go.