...
## Понятие отображения
 ...
 Отображения в Go реализуют идею хеш-таблиц или карт (map). Хеш-таблица — это структура, которая представляет собой неупорядоченный набор пар "ключ — значение". Ключи в хеш-таблице уникальны, и по этим ключам может быть получено значение. Отображения в Go представляют собой ссылку на такую хеш-таблицу. Создать отображение можно с помощью встроенной функции `make`:
 
 ```golang
catsWeights := make(map[string]int)
 ```

 Ключи и значения отображения имеют тип. Для ключей — это может быть один тип, для значений — другой. В данным примере ключи имеют тип `string`, а значения — тип `int`. 

## Заполнение отображения данными
 `catsWeights` — пустое отображение. Вот, что мы увидим, если выведем его через функцию `fmt.Println`:
...
catsWeights["Busa"] = 2
 ...
 ```
map[Barsik:10 Busa:2]
 ```

Заполнить отображение данными можно и при его создании:
```golang 
...
Обратиться к элементу отображения можно по его ключу:
```golang
...
Функция `piDigit` возвращает очередной знак числа Пи после запятой, начиная с нулевого знака и заканчивая девятым включительно. С использованием `piDigit` реализуйте тело функции `countDigits`, которая принимает на вход количество цифр числа Пи после запятой (`0 <= N < 10`) и возвращает отображение с числом каждой из встретившихся цифр. Цифры, не встретившиеся ни разу, в отображение включать не нужно. Например, для `countDigits(5)` результат следующий:
...
```golang {.task_source #golang_chapter_0060_task_0010}
package main
...
Удалить элемент из отображения можно с помощью встроенной функции `delete`:

```golang 
delete(catsWeights, "Busa")
```

Чтобы узнать длину отображения, можно воспользоваться встроенной функцией `len()`:
...
Чтобы узнать число цифр после десятичной точки, можно воспользоваться функцией `Split` пакета `strings`, разбив строку на две: до точки и после точки, а затем вычислить длину строки после точки через встроенную функцию `len()`. {.task_hint}
...
Когда вы обращаетесь к элементу отображения, то всегда получаете значение. Не важно, содержится элемент в отображении или просто имеет значение по умолчанию. В большинстве случае это нормально, однако иногда нужно точно знать, содержит ли отображение данный элемент. На самом деле, когда вы обращаетесь к элементу отображения, то возвращаются само значение отображения и значение типа `bool`. Последнее равно `true`, если элемент содержится в отображении и `false` — в противном случае. Часто его присваивают переменной с именем `ok`. Вот так можно проверить, содержится ли элемент в отображении:
...
Если переменная `ok` не нужна, то объявлять ее не следует, как и раньше.

Как и отображения, срезы нельзя сравнивать друг с другом. Отображение можно сравнить только со значением `nil`. Напишите тело функции `equalWeights`, которая проверяет, содержат ли два среза типа `map[string]int` одинаковые ключи и связанные с ними значения. Функция `equalWeights` возвращает `true`, если это так, и `false` — в противном случае. {.task_text}

```golang {.task_source #golang_chapter_0060_task_0030}
package main
...
## Реализация отображения с уникальными значениями
Во многих современных языках, например в C++ и Python, существуют множества `set`. Когда мы добавляем элементы в множество, то каждый из добавленных элементов оказывается там только один раз. Каждое значение внутри множества уникально. В Go множества отсутствуют, однако для этих целей можно использовать отображения с ключом требуемого типа и значением типа `bool`, которое является флагом уникальности. Попробуйте реализовать эту идею в задаче подсчета уникальных слов в тексте.
...
```golang {.task_source #golang_chapter_0060_task_0040}
package main
...
Создайте отображение типа `map[string]bool`. Добавляйте в цикле каждое слово в качестве ключа отображения, если значение отображения для этого слова `false`, после чего установите его в `true`. Таким образом, вы получите отображение только с уникальными словами. {.task_hint}
...
## Резюме
1. В Go хеш-таблицы реализованы через отображения.
2. Отображение — это ссылка на хеш-таблицу.
3. Ключи и значения отображения типизированы.
4. Отображения заполняются парами "ключ — значение" с уникальными ключами.
5. При попытке узнать значение несуществующего элемента отображения вы получит значение типа по умолчанию. Чтобы узнать, содержится ли элемент в отображении, стоит вспомнить: при обращении к элементу отображения возвращается не только значение отображения, но и флаг. Этот флаг содержит `true`, если элемент содержится в отображении. В противном случае — `false`.
6. Встроенная функция `delete` позволяет удалить элемент из отображения, `len` — узнать длину отображения. Перебрать элементы отображения можно через ключевое слово `range`.
7. Порядок элементов отображения не определен.
8. Отображение с уникальными значениями может быть реализовано стандартными средствами языка Go.