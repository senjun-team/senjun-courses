...
Обычно программист ожидает, что этот код выведет `0 1 2 3 4`. Это может показаться удивительным, но программа, собранная компилятором Go до версии 1.22, выведет `5 5 5 5 5`.
Дело в том, что значения-функции в данном случае «захватывают» переменную `i` и совместно используют адрес этой переменной. 
...
Начиная с версии Go 1.22, такой проблемы больше нет. Изначальный код выводит `0 1 2 3 4`, как и ожидается. 
...
Если мы вызовем эту функцию из `main`, то в памяти образуется массив длиной `100500` элементов. Сборщик мусора никогда не узнает о том, в какой момент этот массив больше вам не нужен. Поэтому он будет в памяти все время, до окончания работы программы. Так что использовать замыкания нужно осторожно. 
