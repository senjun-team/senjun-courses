...
4. Компилятор языка Go обрабатывает управляющие последовательности внутри строки. Например, `\n` — это перенос строки, `\t` — табуляция. Чтобы не обрабатывать эти управляющие последовательности, а работать со строкой, как есть, используйте обратные одинарные кавычки.
5. Существуют различные кодировки. Строки могу содержать произвольные байты. Однако, если строка была сформирована из строковых констант, то она всегда представляется в кодировке UTF-8.
6. 