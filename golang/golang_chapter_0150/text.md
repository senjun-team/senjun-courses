...
Начиная с Go 1.18, в языке появились дженерики — **generics**. Дженерики позволяют писать обобщенный код, способный обрабатывать разные типы данных. Благодаря ним достаточно реализовать алгоритм лишь однажды. После этого его применяют его к разным типам. 
