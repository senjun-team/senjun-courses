...
В этой главе мы познакомимся с некоторыми важными аспектами каналов. 

* Будет рассказано про буферизованные каналы. Они позволяют держать в памяти несколько значений еще до чтения. Это бывает полезно, когда нужно записать много значений, а чтение не выполняется сразу. 
* Рассмотрим тонкий момент, показывающий разницу между небуферизованным каналом и каналом с буфером на один элемент.
* Коснемся вопросов состояния гонки и мьютексов, позволяющих синхронизировать работу горутин.
* Покажем удобный прием с ключевым словом `range`, который облегчает написание кода для чтения из канала.
* Напоследок научимся работать с однонаправленными каналами, облегчающими обнаружение некоторых ошибок.
...
Чтобы получить размер буфера канала, воспользуйтесь встроенной функцией `cap`:

```go
fmt.Println(cap(ch)) // 5
```

Чтобы получить количество элементов в очереди, воспользуйтесь встроенной функцией `len`:
...
В данной случае для пяти задач используется пул из трех соединений. Реализуйте создание нового пула размера `size` через функцию `newPool`. Внутри функции `newPool` необходимо создать буферизованный канал из соединений размером `size`. Инициализируйте соединения идентификаторами `1..size`. {.task_text} 
...
Чтобы этого избежать, используют специальное средство — **мьютекс**. Мьютекс — это примитив синхронизации, который обеспечивает доступ к общему ресурсу только для одной горутины в текущий момент времени. В данном случае нам необходимо ввести мьютекс для структуры `bankAccount`: {#block-mutex}
...
Мы получим тот же результат. Однако хорошей практикой считается все же делать мьютекс частью той структуры, которую он защищает. Благодаря этому приему код становится более наглядным.
...
Некоторые программисты вместо `sync.Mutex` используют канал с буфером на один элемент. Делать так — дурной тон. Лучше все-таки работать с мьютексом, поскольку это явно подчеркивает намерение. Тем не менее, знать о таком способе применения каналов тоже нужно. Такое встречается в чужом коде. Попробуйте реализовать эту идею в данной задаче. {.task_text}

Представленный код позволяет работать с некоторой памятью в виде «ключ — значение». Функция `newMemory` возвращает указатель на новую память. Метод `set` позволяет установить значение памяти по его ключу. Метод `get` возвращает значение памяти по его ключу, а также флаг. Этот флаг равен `true`, если значение есть в памяти. В противном случае — `false`. Метод `modify` модифицирует символ с индексом `i` значения по его ключу. {.task_text}
...
func newMemory() *memoryT {
	c := make(map[string]string)
	return &memoryT{c}
}

func (c *memoryT) set(key string, value string) {
	c.store[key] = value
}

func (c *memoryT) modify(key string, i int, value string) error {
	s, ok := c.get(key)
	if !ok {
...
func (c *memoryT) get(key string) (string, bool) {
...
func main() {
	...
	res, ok := memory.get("code")
	if !ok {
		fmt.Println("error: no value in the memory")
	} else {
		fmt.Printf("res: %s\n", res)
	}
}
...
Добавьте внутри структуры `memoryT` поле `lock` типа `chan struct{}`. Внутри функции `newMemory` запишите в этот канал значение. Это признак того, что «мьютекс» разблокирован. Внутри метода `modify` захватите «мьютекс», прочитав значение. Освободите «мьютекс» после того, как метод отработает. {.task_hint}
...
Рассмотрим пример, в котором мы обрабатываем задачи в отдельной горутине `processTasks`. Результаты она записывает в канал. В горутине `main` мы читаем эти значения из канала, пока `processTasks` его не закроет. 
...
Вместо этого используют ключевое слово `range`:
...
В приведенном коде допущена ошибка. Функция `sendResponse`, которая должна отправить данные в канал, наоборот читает из него. Это приводит к тому, что все горутины «зависают».  
...
Чтобы предотвратить ошибку на этапе компиляции, используют однонаправленные каналы. Они позволяют передавать значение лишь в одну сторону: либо писать, либо читать. Тип `chan<- string` означает канал для типа `string` только на запись, а тип `<-chan string` — только на чтение.

Мы могли бы переписать функцию `sendResponse`: 
...
func main() {
	...
	messages := make(chan string, messageNumber)
	for i := range messageNumber {
		// Для быстрой конкатенации большого числа строк.
		// Изначально builder содержит пустую строку
		var builder strings.Builder
		for range i {
			// Конкатенируем содержимое builder с буквой "А"
			_, err := builder.WriteString("A")
			if err != nil {
				panic(err)
			}
		}
		// Пишем сконкантенированный результат в messages
		messages <- builder.String()
	}
	close(messages)
	const workerNumber = 3
	res := make(chan int, workerNumber)
...
1. Буферизованные каналы позволяют горутинам писать в канал многократно до того, пока будет выполнено первое чтение.
...
3. Канал с буфером на один элемент иногда используют в качестве мьютекса. Однако это плохая практика. Если есть возможность работать с мьютексом, лучше применить мьютекс.
