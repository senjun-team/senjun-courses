...
Максимальное значение `GOGC` зависит от конкретных реализаций и платформы, на которой выполняется программа. На практике гигантские значения `GOGC` не имеют смысла. Для этого есть `off` и `-1`.
