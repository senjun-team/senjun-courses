...
* значение `off` — установить значение `GOGC` бесконечным. Эта опция часто используется для  полного отключения сборщика мусора. Однако одного этого действия не всегда достаточно. Как отключить сборщик мусора полностью — будет рассмотрено далее.
...
Чтобы полностью отключить сборщик мусора, нужно выставить `GOGC` в значение `off`, а `GOMEMLIMIT` — в значение `math.MaxInt64`.
