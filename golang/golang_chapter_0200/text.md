...
Сборщик мусора проходится по этому графу и отмечает те объекты, которые программа использует. Процесс обхода графа называется **сканированием**. После сканирования сборщик мусора проходит по всей памяти в куче и делает доступной всю не помеченную память. 
...
Фиксированное время за цикл — это время на те действия, которые выполняются одинаковое количество раз в каждом цикле. Например, инициализация структур для следующего цикла сборки мусора. Это время сравнительно невелико. 
