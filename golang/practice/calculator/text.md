...
### cast.go
Пакет `cast`, состоящий из единственного файла. Он должен реализовывать работу с AST. Для удобства здесь же можно реализовать функции, выводящие AST на экран. 

### cerrors.go 
Пакет `cerrors`. Здесь должен содержаться список ошибок, которые могут произойти.

### command.go
Пакет `command`. Здесь должна быть описана структура команды. Команда — это арифметическое выражение в трех формах: в сыром виде, в виде токенов и в виде AST.
