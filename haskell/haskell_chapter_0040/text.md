...
```
В объявлении функции укажите, что функция принимает два аргумента типа `Int` и возвращает значение типа `Double`. В определении используйте формулу `b * h / 2.0`. {.task_hint}
```haskell {.task_answer}
module Main where

triangleArea :: Int -> Int -> Double
...
Напишите функцию `clamp`, которая задаёт значение в диапазоне между указанными нижней и верхней границами. Функция принимает три аргумента типа `Int`: минимальное значение `a`, предпочитаемое значение `val` и максимально допустимое `b`. {.task_text}

Для этого воспользуйтесь встроенными функциями `min` и `max`, принимающими два аргумента и возвращающими соответственно минимальный или максимальный из них. {.task_text}
