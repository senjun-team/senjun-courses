...
Тип `LaptopState` имеет мощность 2 * 2 * 2, то есть 8. Она получена **перемножением** количества составляющих тип элементов 3 на количество принимаемых ими значений 2. Поэтому мы в праве сказать, что `LaptopState` — это Тип Произведение (Product Type).
