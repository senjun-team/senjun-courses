...
Заведите тип `Loc`, обозначающий локацию: у него должен быть конструктор от кортежа из двух `Double` (широты и долготы). И для него создайте кастомную функцию `showLoc`, выводящую локацию в виде `"{широта; долгота}"`. {.task_text}

И, наконец, заведите АДТ `Route` (маршрут) с конструктором от `NaviType` (вид передвижения) и `Loc` (точка отправления). Для него также нужна функция `showRoute`. Внутри себя она должна применять `showNaviType` и `showLoc` и возвращать строку вида: {.task_text}
