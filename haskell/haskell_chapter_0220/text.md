...
Функция `segmentLength` получает id географического объекта. Например, тропинки или железной дороги. А также индексы начальной и конечной точки на этом объекте, между которыми требуется получить длину геометрии. {.task_text}
...
Кстати, для `newtype`-типов директива `deriving` применяется точно так же, как для типов, объявленных через `data`! {.task_text}
