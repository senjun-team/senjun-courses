...
- нейтральный элемент: при сложении любого числа с нулем на выходе мы получаем исходное число.
...
В нашем случае мы ходим сказать, что константа `e` это «нулевой элемент», то есть такое специальное значение, для которого будет выполняться следующее:
