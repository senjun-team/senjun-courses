...
- константа — нулевой *элемент* типа `a`,
- бинарная операция типа `a -> a -> a`,
- операция взятия обратного элемента типа `a -> a`.

При этом на операции могут (но не обязательно) накладываться ограничения, называемые свойствами операций или законами (laws): ассоциативность бинарной операции, или тот факт, что нулевой элемент с любым другим элементом, применённые к бинарной операции, дают на выходе последний.
...
В нашем случае, мы ходим сказать, что константа `e` это "нулевой элемент" , т.е. такое специальное значение, для которого будет выполняться следующее:
```haskell
...
`e` - это `0`, `(+)` - это сложение, а `inv` - это минус. 
```
0 + 10 == 10
inv 5 + 5 == 0
```
...
Мы можем написать инстанс `Group (Maybe Bool)` и тогда в качестве `e` мы сможем указать `Nothing` и определить соответствующим образов операции, чтобы все наши ограничения выполнялись. Выше мы определили `Group (Maybe a)` с условием, что `a` реализует класс `Group`. В случае с `Bool` мы не можем так сделать, поэтому придется сделать явную реализацию `Maybe Bool`:
```haskell {.playground}
