...
Вы можете запускать скрипт `tree.py` с разными аргументами командной строки и проверять вывод на различных директориях. Для этого есть кнопка «Запустить» и поле для ввода аргументов. Для удобства тестирования в проекте заведена директория `test_data` со вложенными директориями и файлами. Текущая директория при запуске — директория с именем `project`, в которой и находится `tree.py`.

Когда проект будет готов к тестированию, нажимайте на кнопку «Отправить на проверку». По ней запустятся юнит-тесты из файла `test_tree.py`. В этом же файле можно посмотреть ожидаемый вывод утилиты `tree.py` для различных аргументов командной строки. Для тестирования различных сценариев на время запуска тестов создается пустая директория `historical` внутри `test_data/movies`.
