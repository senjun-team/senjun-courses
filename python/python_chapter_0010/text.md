...
## Что такое питон
Питон — это интерпретируемый язык. Он поддерживает множество парадигм программирования, в первую очередь ООП и функциональное. **Управление памятью** в питоне реализуется через подсчет ссылок и сборщик мусора. 
...
+ **Интроспекция:** в рантайме питона для любого объекта можно выяснить его тип, список методов и полей, родительские классы и прочие ценные свойства.

{.paragraph}

За что питон ругают:
- **Низкое быстродействие.** Следствие интерпретируемости и высокоуровневости. При каждом запуске скрипта интерптератор парсит код и строит AST-дерево. Эту проблему можно сгладить: сохранять промежуточный байт-код в специальных файлах, чтобы не генерировать его заново при каждом перезапуске. 
...
- **Зависимость от системных библиотек.** При установке питон-приложения на целевую машину придется разбираться с настройкой виртуального окружения, изолирующего версии библиотек.
