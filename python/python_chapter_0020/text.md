...
- Оператор моржа `:=` совмещает присвоение значения переменной с возвратом этого значения. Позволяет сократить объем кода и в отдельных случаях оптимизироать работу программы.
