...
**Позиционные аргументы** передаются в функцию в строгой очередности (которая указана в оъявлении функции). Позиционные аргументы идут без указания имен:
```python
...
Представим фукцию, принимающую 4 параметра:
```python
def f(a, b, c, d):
    pass
```
...
Чередование же позиционных и именованных агрументов приведет к ошибке:
```python
f(a = 1, 2, c = 3, d = 4)
```
...
Функция, внутри которой есть `return`, но без возвращаемого значения, либо в которой `return` вовсе отсутствует, на самом деле неявно возвращет `None`. `None` — специальный объект, означающий отсутствие значения:
...
Напишите функцию `calc_gcd(a, b)`, которая находит наибольший общий делитель (GCD, greatest common divisor) чисел `a` и `b`. Функция должна быть рекурсивой и возвращать два значения: GCD и количество рекурсивных вызовов, которые были выполнены перед возвратом ответа. {.task_text}
...
Вместо присваивания ламбды функциональному объекту ее можно сразу вызвать.
```python
(lambda s, n:  s * n)("I", 3)
```
...
Напишите лямбда-функцию, которая просто выводит в консоль "Press any key to continue". Присвойте ее функциональному объекут `to_next_step`.  {.task_text}
