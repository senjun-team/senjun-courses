...
Допустим, консольное приложение ожидает команду и дополнительные опции от пользователя. В строку `user_input` мы читаем пользовательский ввод с помощью встроенной функции `input()`. Затем через метод строки `split()` разбиваем ее по пробелам и получаем список слов. К которому и применяем `match/case`. Форматирование строк осуществляется с помощью [f-строк,](/courses/python/chapters/python_chapter_0100#block-formatting) про которые вы узнаете чуть позже.
