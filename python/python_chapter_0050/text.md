# Глава 5. Встроенные типы данных: строки
...
Этих коллекций хватает для решения многих типовых задач. Но неизбежды ситуации, когда функциональности встроенных типов становится маловато. Тогда в дело вступают библиотечные типы: `array`, `defaultdict`, `deque` и другие. С ними мы познакомимся позже. А пока обсудим каждую из встроенных коллекций.
...
PEP8 рекомендует двойные кавычки `"Some text"` для однострочников и три двойных кавычки ```"""Some text"""``` для многострочных текстов.
...
```python
c = kafka_bootstrap_server[3]
print(c)
```

Этот код выполнится за O(1) и выведет символ по индексу 3:
```python
a
```
...
- `min(s)` — поиск минимального элемента в `s`. O(s).
- `max(s)` — поиск максимального элемента в `s`. O(s).
...
- `s.split()` — разбивает строку по символу-разделителю и возврщает список. По умолчнию разделителем считается пробел, но это переопределяется. Например, `s.split("\t")` разделит строку по символу табуляции.
...
```python {.task_source #python_chapter_0050_task_0060}
```
```python {.task_hint}
```
...
Для того, чтобы в отформатированную строку добавить имя переменной и ее значение, используется компактная записть с именем переменной и `=` внутри фигурных скобок:
```python
x = 5
print(f"{x=}")
```
...
Напишите функцию `f(user_id, visits)`, которая принимает два целых цисла `user_id` и `visits` и возвращает отформатированную строку со статистикой по пользователю: "User '498258' visited page 2 times". {.task_text}
```python {.task_source #python_chapter_0050_task_0070}
```
```python {.task_hint}
```
...
- Для взятия среза используется синтакис `s[start:end:step]`.
