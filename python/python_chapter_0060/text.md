...
## Списки
Список (list) — гетерогенная последовательность, не требущая от элементов уникальности и позволяющая обращаться к ним по индексам. Что-то вроде массива динамической длины: можно модифицировать элементы списка, удалять и добавлять новые.
...
При обращении к элементам списков, вложнных в другие списки, индексы указываются в последовательно идущих квадратных скобках:
```python
...
Удалите выбивющиеся из последовательности элементы. Используйте для этого присвоение слайсу. {.task_text}
...
Для удления одного элемента или слайса элементов из списка используется **оператор del:**
```python
x = ["assert", "bool", "false"]
del x[1]
print(x)
...
- `l.pop(i=-1)` — удаляет из списка `l` элемент по индексу и возврщает его значение. `i` является необязательным параметром со значением -1 по умолчанию.
...
- Тип `tuple` (кортеж) — это неизменяемый список, хотя у иммутабельности есть исключение: если в кортеже хранится элемент изменяемого типа (например, списко), то этот элемент можно модифицировать.
