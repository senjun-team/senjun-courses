...
Напишите функцию `to_fahrenheit(celsius)`. Она должна принимать `celsius` градусов Цельсия и возвращать соответствующее значение по шкале Фаренгейта. {.task_text}
...
```python {.task_source #python_chapter_0070_task_0060}
```
```{.task_hint}
