...
**Пример 5: как увидеть оптимизацию через `dis` и `compile`**

Прежде чем перейти к примеру, разберёмся с двумя понятиями: **байткод** и модуль **`dis`**.
...
Теперь, вооружившись этими знаниями, посмотрим, как CPython обрабатывает одинаковые константы:
...
> **Примечание:** в Python 3.12+ при запуске такого кода вы увидите предупреждение:
> ```
...
А вот тот же код, сохранённый в файл и запущенный как `python script.py`, вернёт `True`:
...
Причина: при компиляции всего файла AST-оптимизатор видит обе строки присваивания и объединяет одинаковые кортежи в один объект. В REPL каждая строка компилируется изолированно, и оптимизатор не может этого сделать.

То же самое касается чисел за пределами диапазона кеширования:
...
**Пример 7: пограничные значения `-6` и `257`**

Диапазон кеширования целых чисел в CPython — от **-5 до 256** включительно. Числа за его пределами ведут себя по-разному в зависимости от контекста:
...
Эти примеры наглядно показывают: **никогда не используйте `is` для сравнения числовых значений**. Результат зависит от версии Python, контекста выполнения (REPL или файл) и конкретных чисел.
