...
Начиная с версии [CPython 3.4](https://docs.python.org/3/whatsnew/3.4.html) ([PEP 454](https://peps.python.org/pep-0445/)), при необходимости добавления нового элемента в список, когда текущего объёма памяти не хватает, используется оптимизированный алгоритм увеличения выделенной памяти. [Формула для вычисления](https://github.com/python/cpython/blob/54a5fdffc8e20f111e7a7d2df352e8be057177ff/Objects/listobject.c#L133) нового объёма выглядит так:

```c
new_allocated = old_size + (old_size >> 3) + 6;
```
Здесь old_size — текущее количество элементов в списке, а `old_size >> 3` эквивалентно целочисленному делению на 8, то есть к текущему размеру добавляется примерно 12,5%. Константа +6 нужна для стабилизации роста при малых размерах списка: без неё при частых добавлениях в короткие списки происходило бы слишком много операций перераспределения памяти, что снижало бы производительность.

Такой подход обеспечивает постепенное, но достаточное увеличение вместимости списка, что минимизирует количество копирований данных и перераспределений памяти. В результате операция добавления элемента (`.append()`) остаётся эффективной с амортизированной временной сложностью O(1).

Однако важно понимать, что отдельный вызов `.append()` не всегда выполняется за O(1). Когда список заполняется до предела выделенной памяти, при добавлении следующего элемента требуется выделить новый блок памяти большего размера, скопировать туда все существующие элементы, а затем добавить новый. Эта операция занимает O(n) времени. Но благодаря тому, что перераспределения происходят всё реже по мере роста списка, общие затраты на большое количество операций `.append()` в среднем остаются пропорциональными количеству добавленных элементов.

Например, если в списке уже 16 элементов и он заполняется полностью, новый объём памяти рассчитывается как:
```commandline
16 + (16 // 8) + 6 = 16 + 2 + 6 = 24
```

Таким образом, под данные выделяется запас в 8 дополнительных слотов, что снижает вероятность следующего перераспределения при ближайших добавлениях.

Чтобы увидеть это поведение в деталях, можно заглянуть внутрь объекта `list` с помощью модуля `ctypes`. Для этого определим класс, соответствующую внутреннему представлению `PyListObject` из cpython:
```python
def get_allocated(lst):
    ...
    return py_list.allocated
```
Теперь проследим, как меняется выделенная память при последовательном добавлении элементов:
```python
lst = []
for i in range(20):
    lst.append(i)
    print(f"len = {len(lst):2}, allocated = {get_allocated(lst)}")
```
На выходе получим:
```commandline
len =  1, allocated = 4
...
len = 17, allocated = 24
```
Видно, что allocated увеличивается не на каждый элемент, а скачкообразно — в соответствии с описанной формулой. Это подтверждает, что `.append()` имеет *амортизированную* сложность O(1).
...
len =  0, allocated = 0
```
Заметно, что при уменьшении размера списка интерпретатор иногда перераспределяет память, уменьшая allocated. Это происходит, когда количество элементов становится значительно меньше выделенного объёма — для экономии памяти. Однако такое перераспределение — не бесплатная операция: оно требует выделения нового блока и копирования оставшихся элементов, то есть выполняется за O(n).
