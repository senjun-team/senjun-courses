...
Без оператора моржа этот же код выглядел бы чуть длиннее. А переменная `q` оказалась бы доступной и за пределами кортежа:
...
```
Для расчета квадратного корня воспользуйтесь функцией `sqrt()` из модуля `math`. Для превращения числа с плавающей точкой в строку, содержащую два знака после запятой, воспользуйтесь f-строкой вида `f"{val:.nf}"`. Здесь `val` — имя переменной, а `n` — количество знаков после запятой. {.task_hint}
```python {.task_answer}
import math
