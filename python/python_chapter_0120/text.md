...
## Оператор is и неизменяемые типы {#block-is}

Оператор `is` может вести себя неожиданно при сравнении неизменяемых типов, в том числе кортежей.
...
Если вы сохраните этот код в файл `.py` и запустите его, то получите `True`. Но если вы выполните те же строки в интерактивном режиме REPL (запустив `python` из командной строки и вводя команды построчно), результат может оказаться `False`.

Это не баг, а особенность реализации CPython. При компиляции целого файла AST-оптимизатор выполняет свёртку констант (constant folding) - он видит, что кортежи идентичны, и, пользуясь их неизменяемостью, создаёт один объект вместо двух. В интерактивном режиме каждая строка компилируется отдельно, поэтому оптимизатор не всегда может объединить одинаковые константы.

В Python 3.7 свертка констант была перенесена из [peephole](https://en.wikipedia.org/wiki/Peephole_optimization)-оптимизатора в новый [AST](https://en.wikipedia.org/wiki/Abstract_syntax_tree)-оптимизатор, что позволило согласованнее применять эту оптимизацию ([bpo-11549](https://bugs.python.org/issue11549), [bpo-29469](https://bugs.python.org/issue29469)). Подробнее об изменениях в оптимизации можно прочитать в [документации Python 3.7](https://docs.python.org/3/whatsnew/3.7.html#optimizations).

Изменяемые объекты (списки, словари, множества) создаются как отдельные объекты, поэтому `is` для них не подходит:
...
Поэтому используйте `is` только для сравнения с синглтонами (`None`, `True`, `False`), а для сравнения значений — оператор `==`.
