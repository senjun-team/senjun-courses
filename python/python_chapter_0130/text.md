...
Ключом словаря может выступать любой хешируемый объект. Сам словарь является изменяемой коллекцией с возможностью удаления и добавления элементов. Как и множество, словарь реализован на базе хеш-таблицы. Поэтому алгоритмическая сложность поиска, добавления и удаления элементов в `dict` — амортизированная константа O(1).
...
Второй способ — избавиться от кавычек и работать с парами ключ-значение как с именованными аргументами функции `dict()`:
```python
product_to_purchases = dict(
    T3234FX=12,
    T048Y11=0,
...
Этот способ сработает, только если ключ-строка удовлетворяет всем требованиям к именованию переменной в питоне (например, не начинается с цифры, не содержит тире и спец-символы).
...
```
```python {.task_hint}
Для проверки на хешируемость воспользуйтесь функцией `isinstance(x, typing.Hashable)`
```
...
На строке 2 произведена попытка обращения к элементам словаря через слайс, а не по ключу. Так как для типов `dict` и `set` операция получения слайса бессмысленна, генерируется исключение:
```
Traceback (most recent call last):
...
Подытожим основные действия над словарями:
...
```python {.task_source #python_chapter_0130_task_0030}
```
```python {.task_hint}
Для проверки ключа на вхождение в словарь используйте оператор `in`, для изменения существующего значения по ключу — `[]`.
```

Но на этом популярные операции над типом `dict` не заканчиваются. Также широко применимы методы:
...
- `d.setdevault(k, default_val)` — получить значение `k`, если этот ключ содержится в словаре. Если не содержится, добавить и вернуть значение `default_val` по этому ключу. `default_val` — опциональный аргумент, по умолчанию `None`.

Удалите из словаря значение 5.5. Используйте для этого один из **методов** `dict`, а не оператор `del`.{.task_text}
```python {.task_source #python_chapter_0130_task_0040}
...
```python {.task_source #python_chapter_0130_task_0050}
```
```python {.task_hint}
...
Не смотря на то, что словари в современном питоне — это упорядоченные коллекции, при сравнении двух словарей оператор `==` вернет `True` вне зависимости от последовательности пар ключ-значение: главное, чтобы они совпадали:
```python
d1 = {1: 1, 2: 2, 3: 3}
d2 = {2: 2, 3: 3, 1: 1}
print(d1 == d2)
