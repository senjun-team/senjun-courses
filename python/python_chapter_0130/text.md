...
Второй способ — избавиться от кавычек и работать с парами ключ-значение как с [именованными аргументами](/courses/python/chapters/python_chapter_0040#block-pos-named-args) функции `dict()`:
```python
product_to_purchases = dict(
    T3234FX=12,
    T048Y11=0,
...
На строке 2 произведена попытка обращения к элементам словаря через [срез,](/courses/python/chapters/python_chapter_0080##block-slices) а не по ключу. Так как для типов `dict` и `set` операция получения среза бессмысленна, генерируется исключение:
```
Traceback (most recent call last):
