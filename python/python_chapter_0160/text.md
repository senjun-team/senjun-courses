...
В прошлых главах мы [намекнули](/courses/python/chapters/python_chapter_0140#block-dunder-attributes) на существование специальных полей и методов класса с зарезервированными именами.
...
```python {.task_source #python_chapter_0160_task_0040}
```
```{.task_hint}
...
```python {.task_source #python_chapter_0160_task_0050}
```
```{.task_hint}
...
```python {.task_source #python_chapter_0160_task_0060}
```
```{.task_hint}
print(list.__dict__)
```
...
```python {.task_source #python_chapter_0160_task_0070}
```
```{.task_hint}
dr.data = "Some data"
```
