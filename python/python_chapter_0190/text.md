# Глава 19. Контекстные менеджеры
...
## Мотивация
Допустим, нам потребовалось открыть файл и дописать в него строку. Как будет выглядеть наивное решение? С помощью встроенной функции `open()` получить объекта файла в режиме на дозапись (append), использовать его и закрыть:
...
- Контекстные менеджеры — более идеоматичная альтернатива для управления ресурсами, чем `try/finally`.
