...
А точнее, `items()` возвращает [генератор,](/courses/python/chapters/python_chapter_0230#block-generators) который при каждом обращении отдает кортеж из двух элементов. Поэтому `items()` безопасно использовать даже для очень больших словарей.
