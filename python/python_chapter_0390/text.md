...
Формат логирования: `"2.03 seconds"` (обратите внимание, что у числа два знака после запятой). {.task_text}
...
Когда вы запускаете `.py`-файл, Python не исполняет ваш исходный код напрямую. Сначала он **компилирует** его в промежуточное представление — **байткод**. [Байткод](https://ru.wikipedia.org/wiki/%D0%91%D0%B0%D0%B9%D1%82-%D0%BA%D0%BE%D0%B4) — это низкоуровневый набор инструкций, понятных виртуальной машине Python. Именно эти инструкции выполняет интерпретатор.

Модуль `dis` (от слова *disassemble* — «дизассемблировать») позволяет «заглянуть внутрь» и показать, в какие байткод-инструкции превратился ваш код. Это полезно, когда хочется понять, почему Python ведёт себя тем или иным образом.

Вооружившись этими знаниями,  проведем эксперимент. Проверим, [считает ли](/courses/python/chapters/python_chapter_0120#block-is) интерпретатор CPython два одинаковых кортежа одним объектом в памяти. 
...
В Python 3.6 вывод выглядел бы так:
...
Обратите внимание: кортеж `(1, 2)` загружается дважды с разными индексами констант (`2` и `3`). Это два разных объекта в памяти.
...
Здесь оба раза используется один и тот же индекс `0` — значит, это один и тот же объект.

Убедиться в этом можно и другим способом — посмотреть на список констант скомпилированного кода. Функция `compile()` принимает исходный код и возвращает объект с байткодом, а атрибут `co_consts` хранит все константы, которые используются в этом коде:

```python
# Python 3.6: кортеж встречается дважды
code = compile('(1, 2) is (1, 2)', '<string>', 'single')
print(code.co_consts)  # (1, 2, None, (1, 2), (1, 2))

# Python 3.7+: кортеж один
code = compile('(1, 2) is (1, 2)', '<string>', 'single')
print(code.co_consts)  # ((1, 2), None)
```

В Python 3.12+ при запуске такого кода вы увидите предупреждение:

```
SyntaxWarning: "is" with 'tuple' literal. Did you mean "=="?
```

Это означает, что сами разработчики языка явно подсказывают: сравнивать литералы через `is` — плохая идея. Даже если технически это «работает», на вас смотрят косо и интерпретатор, и другие разработчики.

Это и есть результат работы AST-оптимизатора: он на этапе компиляции находит одинаковые константы и заменяет их ссылками на один объект.
